from aoc_8.aoc_8 import (
  follow_complex_instructions,
  follow_instructions,
...
class TestSetDict:
  ...
  def test_multi_line(self):
    assert set_dict(["AAA = (BBB, CCC)", "DDD = (EEE, FFF)"]) == {
...
class TestFollowInstructions:
  ...
  def test_two_step_instructions(self):
    assert follow_instructions("LR", {
      "AAA": ("BBB", "CCC"),
      "BBB": ("DDD", "ZZZ")}) == 2

  def test_two_step_single_instruction(self):
    assert follow_instructions("L", {
      "AAA": ("BBB", "CCC"),
      "BBB": ("ZZZ", "DDD")}) == 2
...
class TestComplexDict:
  def test_single_ending_on_a(self):
    assert set_complex_dict(["AAA = (BBB, CCC)"]) == (
      {"AAA": ("BBB", "CCC")},
      {"A": ["AAA"]})

  def test_two_ending_on_a(self):
    assert set_complex_dict(["AAA = (BBB, CCC)", "DDA = (EEE, FFF)"]) == ({
      "AAA": ("BBB", "CCC"),
      "DDA": ("EEE", "FFF")},
      {"A": ["AAA", "DDA"]})
...
class TestComplexFollowInstructions:
  def test_single_step(self):
    assert follow_complex_instructions("R",{
      "AAA": ("BBB", "ZZZ")}, {"A": ["AAA"]}) == 1

  def test_single_step_complex(self):
    assert follow_complex_instructions("L", {
      "AAA": ("ZZZ", "BBB"),
      "BBA": ("BBZ", "CCC")}, {"A": ["AAA", "BBA"]}) == 1
